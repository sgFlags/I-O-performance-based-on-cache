		...
		if (page_has_private(page)) {
			...
			if (!mapping && page_count(page) == 1) {
				...
				if (put_page_testzero(page))
					 ...
					 */
                            //bai start
        //if(page->pg_acct&&page->pg_acct->task->traced){
	...
	for (scan = 0; scan < nr_to_scan && !list_empty(src); scan++) {
		...
		for (; pfn < end_pfn; pfn++) {
           ...
           /* if(cursor_page->pg_acct && cursor_page->pg_acct->task->traced){
                do_gettimeofday(&tv);
                spin_lock(&cursor_page->pg_acct->task->acct->lock);
                min_pages_in_lru = cursor_page->pg_acct->task->acct->total_pages/timeval_minus(tv,cursor_page->pg_acct->task->acct->last_time_val)*10*HZ;//计算预留内存大小
                if(cursor_page->pg_acct->task->acct->total_pages_in_lru <= min_pages_in_lru){
                     spin_unlock(&cursor_page->pg_acct->task->acct->lock);
                     break;
                }
                else{
                    cursor_page->pg_acct->task->acct->total_pages_in_lru--;
                    spin_unlock(&cursor_page->pg_acct->task->acct->lock);
                }
            }*/
             //bai end
